...
class FilterView(LeadView, ListView):
    ...
    def post(self, request):
        ...
        for key, value in request.POST.items():
            ...
            if 'selectfields' in key: # I created some hidden input elements in view.html, whose names are just the rest names with 'selectfields attached. This allowed me to easily get a list of field rest names right here.
                fields.append(value)
        ...
        for filterkey, filterval in filters.iteritems():
            csvdict.update({filterkey: [""]}) # This just adds the rest names present in 'flters' to the top of csvdict, so they can be the headers for the CSV
        ...
        for lead in leader:
            ...
            while loopstatus == 0: # Can a lead is disqualified, loopstatus becomes nonzero to break the loop
                                ...
                                for filterkey, filterval in filters.iteritems():
                                    if filterkey == dockey: # Match keys between 'filters' and 'document'
                                        if filterval[0] == 0: # range variables
                                            ...
                                            csvrow.clear()
                            else: csvrow.clear(); loopstatus = 1 # if 'document' value is null
